...
try:
    while True:
        ...
        if hand_results.multi_hand_landmarks:
            ...
            thumbs_extended = 0  # Count extended thumbs
            pinkies_extended = 0  # Count extended pinkies
            ...
            for hand_idx, hand_landmarks in enumerate(hand_results.multi_hand_landmarks):
                ...
                if handedness == "Left" and index_extended:
                    left_index_extended = True
                elif handedness == "Right" and index_extended:
            ...
            # Handle A key (left index)
            if left_index_extended and not pressing_a:
                keyboard.press('a')
                pressing_a = True
                print("Pressing A")
            elif not left_index_extended and pressing_a:
                keyboard.release('a')
                pressing_a = False
                print("Releasing A")
                
            # Handle D key (right index)
            if right_index_extended and not pressing_d:
                keyboard.press('d')
                pressing_d = True
                print("Pressing D")
            elif not right_index_extended and pressing_d:
                keyboard.release('d')
                pressing_d = False
                print("Releasing D")
                
            # Handle W key (both thumbs)
            if thumbs_extended >= 2 and not pressing_w:
                keyboard.press('w')
                pressing_w = True
                print("Pressing W")
            elif thumbs_extended < 2 and pressing_w:
                keyboard.release('w')
                pressing_w = False
                print("Releasing W")
                
            # Handle S key (both pinkies)
            if pinkies_extended >= 2 and not pressing_s:
                keyboard.press('s')
                pressing_s = True
                print("Pressing S")
            elif pinkies_extended < 2 and pressing_s:
                keyboard.release('s')
                pressing_s = False
                print("Releasing S")
